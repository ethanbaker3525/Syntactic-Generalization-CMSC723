...
simpleClean["+gap direction"] = simpleClean["+gap effect"].apply(lambda x: "p" if x > 0 else ("n" if x < 0 else 'zero'))
simpleClean.to_csv(f"{output}/simpleFGE.tsv", sep="\t")
...
simpleCleanWithLME.to_csv(f"{output}/simpleAllInfo.tsv", sep="\t")
...
simpleCorrect.to_csv(f"{output}/simpleCorrect.tsv", sep="\t")
...
islandClean["+gap direction"] = islandClean["+gap effect"].apply(lambda x: "p" if x > 0 else ("n" if x < 0 else 'zero'))
islandClean.to_csv(f"{output}/islandFGE.tsv", sep="\t")
...
islandCleanWithLME.to_csv(f"{output}/islandAllInfo.tsv", sep="\t")
...
islandCorrect.to_csv(f"{output}/islandCorrect.tsv", sep="\t")
